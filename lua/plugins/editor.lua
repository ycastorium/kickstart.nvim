  ...
  },
  {
    ...
    build = function()
      ...
      if not TS.get_installed then
        Log.error 'Please restart Neovim and run `:TSUpdate` to use the `nvim-treesitter` **main** branch.'
        return
      end
      package.loaded['lazyvim.util.treesitter'] = nil
      TreeSitterUtil.build(function()
    ...
    config = function(_, opts)
      ...
      if not TS.get_installed then
        return Log.error 'Please use `:Lazy` and update `nvim-treesitter`'
      elseif type(opts.ensure_installed) ~= 'table' then
        return Log.error '`nvim-treesitter` opts.ensure_installed must be a table'
      end

      -- setup treesitter
      TS.setup(opts)
      TreeSitterUtil.get_installed(true) -- initialize the installed langs

      -- install missing parsers
      local install = vim.tbl_filter(function(lang)
        return not TreeSitterUtil.have(lang)
      end, opts.ensure_installed or {})
      if #install > 0 then
        TreeSitterUtil.build(function()
          TS.install(install, { summary = true }):await(function()
            TreeSitterUtil.get_installed(true) -- refresh the installed langs
          end)
        end)
      end

      vim.api.nvim_create_autocmd('FileType', {
        group = vim.api.nvim_create_augroup('castorvim_treesitter', { clear = true }),
        callback = function(ev)
          local ft, lang = ev.match, vim.treesitter.language.get_lang(ev.match)
          if not TreeSitterUtil.have(ft) then
            return
          end

          local function enabled(feat, query)
            local f = opts[feat] or {}
            return f.enable ~= false and not (type(f.disable) == 'table' and vim.tbl_contains(f.disable, lang)) and TreeSitterUtil.have(ft, query)
          end
          ...
          if enabled('indent', 'indents') then
            Helpers.set_default('indentexpr', 'v:lua.LazyVim.treesitter.indentexpr()')
          end

          -- folds
          if enabled('folds', 'folds') then
            if Helpers.set_default('foldmethod', 'expr') then
              Helpers.set_default('foldexpr', 'v:lua.LazyVim.treesitter.foldexpr()')
            end
          end
        end,
      })
